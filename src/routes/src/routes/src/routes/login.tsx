...
function LoginPage() {
  ...
  const handleOtpChange = (i:number, val:string) => {
    const digit=val.replace(/\D/g,"").slice(-1); const next=[...otp]; next[i]=digit; setOtp(next);
        ...
        <div className="depth-card rounded-[1.75rem] p-7 overflow-hidden">
          <AnimatePresence mode="wait" initial={false}>
            ...
            {flow==="email"&&(<motion.div key="email" initial={{opacity:0,x:16}} animate={{opacity:1,x:0}} exit={{opacity:0,x:16}} transition={{duration:0.22}}>
              ...
              <p className="text-sm text-muted-foreground mt-1 mb-6">{mode==="signin"?"Enter your email and password.":"Choose a password to secure your account."}</p>
              <form onSubmit={submitEmail} className="space-y-3">
                <input type="email" required autoFocus value={email} onChange={e=>setEmail(e.target.value)} placeholder="Email" className={inputCls}/>
                <input type="password" required minLength={6} value={password} onChange={e=>setPassword(e.target.value)} placeholder="Password" className={inputCls}/>
                <button type="submit" disabled={!!busy} className={btnPrimary}>{busy==="email"?<Spinner light/>:mode==="signin"?"Sign in →":"Create account →"}</button>
              </form>
            ...
            {flow==="phone"&&(<motion.div key="phone" initial={{opacity:0,x:16}} animate={{opacity:1,x:0}} exit={{opacity:0,x:16}} transition={{duration:0.22}}>
              <button onClick={()=>setFlow("main")} className="text-xs text-muted-foreground hover:text-foreground mb-5 inline-flex items-center gap-1">← Back</button>
              <h1 className="font-display text-2xl font-bold">Your phone</h1>
              <div className="space-y-3 mt-6"><div className="flex gap-2"><div className="rounded-xl border border-border bg-input px-3 py-3 text-sm text-muted-foreground font-mono">+1</div><input type="tel" autoFocus value={phone} onChange={e=>setPhone(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendOtp()} placeholder="(555) 000-0000" className="flex-1 rounded-xl border border-border bg-input px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-ring transition"/></div><button onClick={sendOtp} disabled={!!busy} className={btnPrimary}>{busy==="phone"?<Spinner light/>:"Send Code →"}</button></div>
            </motion.div>)}
            {flow==="otp"&&(<motion.div key="otp" initial={{opacity:0,x:16}} animate={{opacity:1,x:0}} exit={{opacity:0,x:16}} transition={{duration:0.22}}>
              <button onClick={()=>{setOtp(["","","","","",""]);setFlow("phone");}} className="text-xs text-muted-foreground hover:text-foreground mb-5 inline-flex items-center gap-1">← Back</button>
              ...
              <div className="space-y-4"><div className="flex gap-2 justify-center">{otp.map((digit,i)=>(<input key={i} ref={otpRefs[i]} type="text" inputMode="numeric" maxLength={1} value={digit} autoFocus={i===0} onChange={e=>handleOtpChange(i,e.target.value)} onKeyDown={e=>{if(e.key==="Backspace"&&!digit&&i>0)otpRefs[i-1].current?.focus();if(e.key==="Enter"&&otp.join("").length===6)verifyOtp();}} className="w-11 h-12 rounded-xl border border-border bg-input text-center text-xl font-mono font-bold outline-none focus:ring-2 focus:ring-ring transition"/>))}</div><button onClick={verifyOtp} disabled={otp.join("").length<6||!!busy} className={btnPrimary}>{busy==="otp"?<Spinner light/>:"Verify →"}</button><p className="text-xs text-center text-muted-foreground">Didn't get it? <button onClick={()=>{setOtp(["","","","","",""]);setFlow("phone");}} className="underline">Try again</button></p></div>
            </motion.div>)}
          </AnimatePresence>
        </div>
